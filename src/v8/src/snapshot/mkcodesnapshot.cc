...
class SnapshotWriter {
 public:
  ...
  void WriteStartupBlob(v8::StartupData blob) const {
    i::Vector<const i::byte> blob_vector(
        reinterpret_cast<const i::byte*>(blob.data), blob.raw_size);
    if (!startup_blob_file_){
      i::PrintF("startup blob file is not set\n");
      return;
    }
    size_t written = fwrite(blob_vector.begin(), 1, blob_vector.length(), startup_blob_file_);
    if (written != static_cast<size_t>(blob_vector.length())) {
...
int main(int argc, char** argv) {
  ...
  if (result > 0 || (argc != 2) || i::FLAG_help) {
    ::printf("Usage: %s [flag] ... JSfile\n", argv[0]);
    ::printf("     : Output will be codesnapshot.JSfile\n");
  ...
  {
    char snapshot_name[50] = "codesnapshot.";
    strcat(snapshot_name,argv[1]);
    
    SnapshotWriter writer(snapshot_name);
    // set blob file by opening given named file
    i::FLAG_startup_blob = snapshot_name;
    writer.SetStartupBlobFile(snapshot_name);
    //if (i::FLAG_startup_blob) writer.SetStartupBlobFile(i::FLAG_startup_blob);
    char* extra_code;

    i::PrintF("--Making snapshot with extra code : %s\n",argv[1]);
    i::PrintF("--Output filename is : %s\n",snapshot_name);
    extra_code = GetExtraCode(argv[1]);

    //StartupData blob = v8::V8::CreateSnapshotDataBlob(extra_code);
    StartupData blob = v8::V8::CreateCodeSnapshotDataBlob(argv[1], extra_code);
    CHECK(blob.data);
    writer.WriteStartupBlob(blob);
    delete[] extra_code;
    delete[] blob.data;
  }
...
#else
int main(int argc, char** argv){
  i::PrintF("VMOLAB_CODESNAPSHOT is set to false; mkcodesnapshot is disabled\n");
}
#endif

...
v8::StartupData Snapshot::CreateCodeSnapshotBlob(
    const i::List<byte>& payload,
    const i::CodeSerializer& cs, bool embeds_script) {
  //creates Metadata
  i::Snapshot::Metadata metadata;
  metadata.set_embeds_script(true);
  
  i::SerializedCodeData code_snapshot(payload, cs);
  //SnapshotData startup_snapshot(startup_ser);
  //SnapshotData context_snapshot(context_ser);
  Vector<const byte> code_data = code_snapshot.RawData();

  int code_length = code_data.length();

  int length = kCodeDataOffset + code_length;
  char* data = new char[length];

  memcpy(data + kMetadataOffset, &metadata.RawValue(), kInt32Size);
  memcpy(data + kCodeLengthOffset, &code_length, kInt32Size);
  memcpy(data + kCodeDataOffset, code_data.begin(), code_length);
  v8::StartupData result = {data, length};

  if (FLAG_profile_deserialization) {
    PrintF(
        "CodeSnapshot blob consists of:\n %10d bytes for code\n",
        code_length);
  }
  return result;
}

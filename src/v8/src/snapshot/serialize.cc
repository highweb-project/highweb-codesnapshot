...
#if VMOLAB_CODESNAPSHOT
StartupData CodeSerializer::SerializeForSnapshot(Isolate* isolate,
                                      const char* script_name,
                                      const char* custom_source,
                                      bool embeds_script) {
  HandleScope scope(isolate);
  Handle<String> source = isolate->factory()->NewStringFromUtf8(CStrVector(custom_source)).ToHandleChecked();
  Handle<String> name = isolate->factory()->NewStringFromUtf8(CStrVector(script_name)).ToHandleChecked();
  
  PrintF("VMOLAB_CODESNAPSHOT : name is %s\n",script_name);
  ...
  Handle <SharedFunctionInfo> orig = Compiler::CompileScript(
    ...
    Handle<Context>(isolate->context()), NULL, &script_data, v8::ScriptCompiler::kProduceCodeCache,
    NOT_NATIVES_CODE, false);
  
  v8::StartupData code_data = {reinterpret_cast<const char*>(script_data->data()), script_data->length()};
  PrintF("VMOLAB_CODESNAPSHOT : data length is %d\n",script_data->length());
...
#if VMOLAB_CODESNAPSHOT
void SerializedCodeData::PrintHeaders() const {
  uint32_t magic_number = GetMagicNumber();
  uint32_t version_hash = GetHeaderValue(kVersionHashOffset);
  uint32_t source_hash = GetHeaderValue(kSourceHashOffset);
  uint32_t cpu_features = GetHeaderValue(kCpuFeaturesOffset);
  ...
  uint32_t c1 = GetHeaderValue(kChecksum1Offset);
  uint32_t c2 = GetHeaderValue(kChecksum2Offset);
  PrintF("SerializedCodeData::PrintHeaders\n");
  PrintF("\tmagic_number = %x\n\tversion_hash = %x\n\tsource_hash = %x\n\tcpu_features = %x\n",
          magic_number,version_hash,source_hash,cpu_features);
  PrintF("\tflags_hash = %x\n\tChecksum1 = %x\n\tChecksum2 = %x\n",flags_hash,c1,c2);
}
#endif

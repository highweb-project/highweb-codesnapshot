...
StartupData __attribute__((optnone)) V8::CreateCodeSnapshotDataBlob(const char* script_name, const char* custom_source) {
  i::Isolate* internal_isolate = new i::Isolate(true);
  ArrayBufferAllocator allocator;
  internal_isolate->set_array_buffer_allocator(&allocator);
  Isolate* isolate = reinterpret_cast<Isolate*>(internal_isolate);
  StartupData result = {NULL, 0};
  {
    ...
    internal_isolate->Init(NULL);
    Persistent<Context> context;
    //metadata will be made later or do not needed
    //i::Snapshot::Metadata metadata;
    bool embeds_script = false;
    {
      HandleScope handle_scope(isolate);
      Local<Context> new_context = Context::New(isolate);
      internal_isolate->set_creating_default_snapshot(false);
      ...
      if (custom_source != NULL) {
        ...
        embeds_script = true;
        Context::Scope context_scope(new_context);
        // Run given extra code at here
        //if (!RunExtraCode(isolate, new_context, custom_source)) context.Reset();
        // Compile and serialize code
        result = i::CodeSerializer::SerializeForSnapshot(internal_isolate,
            script_name,
            custom_source,
            embeds_script);
      }
      else{
        i::PrintF("No JS file is given for creating code snapshot.\n");
        return result;
      }
    }
    if (!context.IsEmpty()) {
...
void TimeStamp(const char* message) {
  static long start_time = v8::myclock();
  long time = myclock();
  long dt = time-start_time;
  printf("[%d->%d] %s = %3ld.%03ld\n",getppid(),getpid(),message,dt/1000,dt % 1000);
}
}
#endif

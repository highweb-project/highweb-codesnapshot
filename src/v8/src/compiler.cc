...
Handle<SharedFunctionInfo> Compiler::CompileScript(
...
#if VMOLAB_CODESNAPSHOT
  // if codeDeser is on
  // snapshot file of script name exists
  PrintF("extension : %p\n",extension);
  if(FLAG_vmo_codeDeser && natives != NATIVES_CODE && extension == NULL){
    v8::StartupData startup_data = {NULL, 0};
    base::SmartArrayPointer<char> name_smart = String::cast(*script_name)->ToCString();
    char* name = name_smart.get();
    // erase path to get file name (file_name.js)
    char* filename = name;
    for(int i=0 ; name[i]!='\0' ; i++){
      if(name[i]=='/') filename = &name[i+1];
    }
    char snapshot_path[50] = "snapshot/codesnapshot.";
    strcat(snapshot_path,filename);
    // load it to the memory. snapshot is saved as StartupData type.
    PrintF("VMOLAB_CODESNAPSHOT: finding snapshot file from %s\n",snapshot_path);
    FILE* file = fopen(snapshot_path,"rb");
    if (!file){
      PrintF("VMOLAB_CODESNAPSHOT: snapshot file not found\n");
    }
    else{
      ...
      int read_size = static_cast<int>(fread(const_cast<char*>(startup_data.data), 1,
                                              startup_data.raw_size, file));
      fclose(file);

      if(startup_data.raw_size != read_size){
        PrintF("VMOLAB_CODESNAPSHOT: After snapshot file loading : size does not match.\n");
      }
      // make script data out of loaded snapshot
      ScriptData* script_data = new ScriptData(reinterpret_cast<const byte*>(startup_data.data),
                                              startup_data.raw_size);
      // set cached_data and compile_options to use loaded snapshot later in this call.
      cached_data = &script_data;
      compile_options = ScriptCompiler::kConsumeCodeCache;
      PrintF("VMOLAB_CODESNAPSHOT: snapshot file is loaded from %s\n",snapshot_path);
    }
  }
#endif 
  int source_length = source->length();
...
#if VMOLAB_CODESNAPSHOT
  if(FLAG_vmo_debug){
    PrintF("CompileScript is called\n");
    PrintF("FLAG_serialize_toplevel %d\n",FLAG_serialize_toplevel);
    PrintF("compile_options %d\n",compile_options);
    PrintF("\t\t : for CodeSer, must be kProduceCodeCache(%d)\n",
        ScriptCompiler::kProduceCodeCache);
    PrintF("\t\t : for CodeDeser, must be kProduceCodeCache(%d)\n",
  ...
  if (extension == NULL) {
    ...
    if (maybe_result.is_null() && FLAG_serialize_toplevel &&
      ...
      Handle<SharedFunctionInfo> result;
#if VMOLAB_TIME
      v8::TimeStamp("Deserializing Code:Start");
#endif
      if (CodeSerializer::Deserialize(isolate, *cached_data, source)
              .ToHandle(&result)) {
#if VMOLAB_TIME
        v8::TimeStamp("Deserializing Code:End");
#endif
  ...
  if (!maybe_result.ToHandle(&result)) {
    ...
    if (compile_options != ScriptCompiler::kNoCompileOptions) {
      parse_info.set_cached_data(cached_data); } parse_info.set_compile_options(compile_options);
...
#if VMOLAB_TIME
    if(natives != NATIVES_CODE)
      v8::TimeStamp("Compiling Code:Start(CompileToplevel)");
#endif
    result = CompileToplevel(&info);
#if VMOLAB_TIME
    if(natives != NATIVES_CODE)
      v8::TimeStamp("Compiling Code:End(CompileToplevel)");
#endif
...
#if VMOLAB_CODESNAPSHOT
        if(FLAG_vmo_debug) PrintF("Serializeing Code\n");
#endif
